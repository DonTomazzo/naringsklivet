...
interface Fraga {
  ...
  kategori: string;
  rubrik: string;        // ← ny
  rubrikOrange: string;  // ← ny (del som ska vara orange)
  bubbla: string;
  fraga: string;
...
const fragor: Fraga[] = [
  ...
  },
  {
    ...
    alternativ: [
      ...
      { text: 'Nej — alla medlemmar ska behandlas lika utan godtyckliga undantag', korrekt: true, feedback: 'Rätt. Likhetsprincipen innebär att styrelsen inte får missgynna en enskild medlem utan sakliga skäl.' },
      ...
      { text: 'Det beror på om markiserna ser likadana ut', korrekt: false, feedback: 'Utseendet avgör inte saken. Frågan är om omständigheterna är de samma.' },
    ],
    tips: [
  ...
  },
  {
    ...
    alternativ: [
      ...
      { text: 'Stamledningar och sådana som betjänar flera lägenheter är föreningens ansvar', korrekt: true, feedback: 'Rätt. Ledningar som ingår i stammen eller betjänar fler lägenheter = föreningens ansvar.' },
      { text: 'Det avgörs av hur gammal ledningen är', korrekt: false, feedback: 'Ålder avgör inte ansvaret. Det är ledningens funktion som räknas.' },
    ],
    tips: [
  ...
  },
  {
    ...
    alternativ: [
      { text: 'Ja — om alla ledamöter är eniga', korrekt: false, feedback: 'Fel. Ett justerat och undertecknat protokoll är en officiell handling. Enighet ändrar inte det.' },
      { text: 'Ja — sekreteraren kan alltid rätta felaktigheter', korrekt: false, feedback: 'Uppenbara felskrivningar kan rättas, men substansen får inte ändras utan formellt beslut.' },
      { text: 'Nej — fel i ett justerat protokoll rättas via ett nytt beslut på nästa möte', korrekt: true, feedback: 'Rätt. Det justerade protokollet är oföränderligt. Rättelser hanteras formellt på nästa möte.' },
      { text: 'Det spelar ingen roll — protokoll är interna dokument utan juridisk betydelse', korrekt: false, feedback: 'Fel. Protokoll är officiella handlingar som kan behövas vid tvister och revisioner.' },
    ],
    tips: [
  ...
  },
  {
    ...
    alternativ: [
      { text: 'Ja — alla som bor i huset har rösträtt', korrekt: false, feedback: 'Fel. Det är inte boende i huset som ger rösträtt, utan formellt medlemskap.' },
      { text: 'Ja — sambor är automatiskt medlemmar', korrekt: false, feedback: 'Fel. Sambor är inte automatiska medlemmar.' },
      { text: 'Nej — rösträtt kräver formellt medlemskap, och sambor är inte automatiskt medlemmar', korrekt: true, feedback: 'Rätt. Bara den som är registrerad som medlem har rösträtt.' },
  ...
  },
  {
    ...
    alternativ: [
      ...
      { text: 'Nej — åtgärder som väsentligt förändrar föreningens ekonomi kräver stämmobeslut', korrekt: true, feedback: 'Rätt. Beslut som väsentligt förändrar föreningens ekonomiska situation kräver stämmans mandat.' },
      { text: 'Det beror på om summan överstiger 5 miljoner', korrekt: false, feedback: 'Det finns ingen generell beloppsgräns i lagen. Det avgörande är om åtgärden är ordinär förvaltning.' },
    ],
    tips: [
  ...
  },
  {
    ...
    alternativ: [
      { text: 'Förvaltaren — de har mandat att agera självständigt', korrekt: false, feedback: 'Fel. Förvaltaren arbetar på styrelsens uppdrag, inte med självständigt mandat.' },
      { text: 'Ansvaret delas lika mellan styrelsen och förvaltaren', korrekt: false, feedback: 'Fel. Det juridiska ansvaret vilar alltid på styrelsen.' },
      { text: 'Styrelsen — de har alltid det yttersta juridiska ansvaret', korrekt: true, feedback: 'Rätt. Det är alltid styrelsen som bär det yttersta ansvaret för vad som sker i föreningens namn.' },
...
export default function BrfMissuppfattningsQuiz({
  ...
  const handleNästa = () => {
    ...
    else { setFas('result'); onComplete?.('brf-missuppfattningar'); }
  };

  const handleOm = () => {
    setFas('intro'); setIdx(0); setValt(null);
  ...
  // ── INTRO ──────────────────────────────────────────────
  if (fas === 'intro') return (
    <div className="h-full relative overflow-hidden flex items-center justify-center" style={{ background: '#0f1623' }}>
      <img src={bakgrundsbild} alt="" className="absolute inset-0 w-full h-full object-cover" style={{ opacity: 0.25 }} />
      <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg,rgba(15,22,35,0.97) 0%,rgba(15,22,35,0.85) 100%)' }} />
      <div className="relative z-10 max-w-lg mx-auto px-8 py-12 text-center">
        ...
        <h2 className="text-4xl font-black text-white mb-4 leading-tight" style={{ fontFamily: "'Nunito', sans-serif" }}>
          Är du redo att vässa dina <span style={{ color: O }}>styrelsekunskaper?</span>
        </h2>
        <p className="text-white/50 text-base leading-relaxed mb-8">
          Dessa är de vanligaste misstagen och missuppfattningarna i bostadsrättsföreningar.
        </p>
        <div className="grid grid-cols-3 gap-3 mb-8">
          {[{ val: `${fragor.length}`, label: 'Situationer' }, { val: '~8 min', label: 'Tid' }, { val: 'Praktisk', label: 'Fokus' }].map((s, i) => (
        ...
        {isDone && (
          <div className="mb-5 rounded-xl px-4 py-3 text-sm font-semibold" style={{ background: `${O}20`, color: O, border: `1px solid ${O}40` }}>
  ...
  if (fas === 'result') {
    ...
    return (
      <div className="h-full relative overflow-hidden flex items-center justify-center" style={{ background: '#0f1623' }}>
        <img src={bakgrundsbild} alt="" className="absolute inset-0 w-full h-full object-cover" style={{ opacity: 0.2 }} />
        ...
        <div className="relative z-10 max-w-md mx-auto px-8 py-12 text-center space-y-6">
          ...
          <div className="flex justify-center gap-2">
            {svar.map((r, i) => <div key={i} className="w-3 h-3 rounded-full" style={{ background: r ? O : 'rgba(255,255,255,0.2)' }} />)}
          </div>
          ...
          {onNext && (
            <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={onNext}
              ...
              style={{ background: `linear-gradient(135deg,${O},${OD})`, boxShadow: `0 8px 28px ${O}45` }}>
  ...
  <AnimatePresence mode="wait">
    <motion.div key={idx} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
      ...
      <h2 style={{
        ...
        {current.rubrik}<br />
        <span style={{ color: O }}>{current.rubrikOrange}</span>
      </h2>
      ...
      <div style={{ padding: '40px 48px', borderRadius: 28, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', backdropFilter: 'blur(20px)', width: '100%', boxShadow: '0 24px 48px rgba(0,0,0,0.4)' }}>
        ...
        {/* HÖGER */}
        <div style={{ background: '#FAFAF8', overflowY: 'auto', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', padding: '160px 44px 36px' }}>
          <AnimatePresence mode="wait">
            {!visar ? (
      ...
      {/* ── MOBIL ── */}
      <div style={{ display: isDesktop ? 'none' : 'flex', flex: 1, flexDirection: 'column', justifyContent: 'center', position: 'relative', zIndex: 10, overflowY: 'auto' }}>

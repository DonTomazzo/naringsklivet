...
const Hero = () => {
  ...
  return (
    <section className="relative pt-28 pb-16 overflow-hidden min-h-screen flex flex-col justify-between">
      {/* 1. BAKGRUNDSVIDEO */}
      <video
        autoPlay
        loop
        muted
        playsInline
        poster="/hero-fallback.jpg" /* Valfri fallback-bild innan videon laddat */
      ...
      {/* 2. OVERLAY / MÖRK TONING
          Ger mörk bakgrund så den vita och orangea texten syns tydligt */}
      <div 
        className="absolute inset-0 z-0"
        style={{ 
          background: `linear-gradient(180deg, ${C.dark}EE 0%, rgba(15, 23, 42, 0.75) 50%, ${C.dark}FA 100%)` 
        }} 
      />

      {/* 3. INNEHÅLL (Placerat över videon via z-10) */}
      <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-8 w-full">
        
        {/* Soft orange glödeffekt i bakgrunden */}
        <div 
          className="absolute top-0 right-0 w-[600px] h-[600px] rounded-full pointer-events-none opacity-20"
          style={{ background: `radial-gradient(circle, ${C.orange} 0%, transparent 70%)`, transform: 'translate(30%, -20%)' }} 
        />
        ...
        <motion.div 
          ...
          className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest mb-8 border border-white/10 backdrop-blur-md"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start">
          ...
          >
            <h1 
              className="text-5xl sm:text-7xl lg:text-7xl font-black leading-[0.95] tracking-tight mb-8 text-white"
            ...
            <p className="text-lg sm:text-xl leading-relaxed mb-8 max-w-lg text-slate-300">
              Det enda träningsprogrammet i Sverige skräddarsytt för förtroendevalda i bostadsrättsföreningar.
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <motion.button 
                ...
                whileTap={{ scale: 0.97 }}
                onClick={() => navigate('/seminarier')}
                className="inline-flex items-center justify-center gap-2.5 px-8 py-4 rounded-2xl font-bold text-base text-white border border-white/20 backdrop-blur-md hover:bg-white/10"
          ...
          {/* Höger kolumn - Grundarbild med Badge */}
          <motion.div 
          ...
          >
            <div className="relative">
              <div className="rounded-3xl overflow-hidden aspect-[4/5] bg-slate-800 shadow-2xl border border-white/10">
                <img 
                  src="/images/Snapshot.jpg" 
                  alt="Tomas Mauritzson — Styrelsekörkortet"
                  className="w-full h-full object-cover object-top" 
                />
              </div>

              {/* Flytande badge med glassmorphism */}
              <div className="absolute -bottom-5 -right-5 bg-slate-900/90 backdrop-blur-xl rounded-2xl px-5 py-4 shadow-2xl border border-white/15">
                <p className="text-xs font-bold uppercase tracking-widest mb-0.5" style={{ color: C.orange }}>
                  Styrelsekörkortet®
                </p>
                <p className="text-sm font-bold text-white">164+ utbildade styrelser</p>
              </div>

              <div className="absolute -top-6 -left-6 w-32 h-32 rounded-full opacity-20 -z-10" style={{ background: C.orange }} />
            </div>
          </motion.div>
        </div>
        ...
        <motion.div 
          ...
          className="w-full mt-24 p-8 sm:p-12 rounded-3xl bg-slate-900/60 backdrop-blur-xl border border-white/10"
        >
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 items-center">
            <div className="lg:col-span-3 text-center lg:text-left">
              <span 
                className="inline-block text-xs font-bold uppercase tracking-widest px-3 py-1.5 rounded-full mb-6 text-white"
                style={{ background: C.orange }}
              >
                Varför Styrelsekörkortet?
              </span>

              <h2 
                className="text-3xl sm:text-5xl font-bold mb-8 leading-tight text-white"
                style={{ fontFamily: "'Nunito', sans-serif" }}
              >
              ...
              <div className="max-w-3xl mx-auto lg:mx-0 space-y-6 text-lg text-slate-300 leading-relaxed">
                <p>
              ...
              <div className="mt-10">
                <button 
                  onClick={() => navigate('/modules')}
                  className="inline-flex items-center gap-2 font-bold text-base px-8 py-4 rounded-xl text-white transition-transform hover:scale-105 shadow-lg"
            ...
            <div className="lg:col-span-1 flex justify-center">
              <div className="relative w-full max-w-[200px] lg:max-w-full">
                <div className="rounded-2xl overflow-hidden shadow-2xl border-2 border-white/20 rotate-3 hover:rotate-0 transition-transform duration-300">
                  <img src="/founder3.png" alt="" className="w-full h-auto object-cover" />
                </div>
                <div className="absolute -z-10 -bottom-3 -left-3 w-12 h-12 rounded-full opacity-30" style={{ background: C.orange }} />
              </div>
            </div>
          </div>
        </motion.div>

        {/* Nedersta kortet med Unsplash-bilden */}
        <motion.div 
          initial={{ opacity: 0, y: 32 }} 
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, delay: 0.36 }}
          className="mt-12 rounded-3xl overflow-hidden relative border border-white/10"
          style={{ height: '380px' }}
        >
          <img 
            src="https://images.unsplash.com/photo-1542744173-8e7e53415bb0?w=1600&q=85"
            alt="BRF-styrelsemöte" 
            className="w-full h-full object-cover object-center" 
          />
          <div 
            className="absolute inset-0"
            style={{ background: `linear-gradient(to right, ${C.dark}CC 0%, transparent 60%, ${C.orange}33 100%)` }} 
          />
          <div 
            className="absolute bottom-6 left-6 sm:left-10 flex items-center gap-3 px-5 py-3.5 rounded-2xl border border-white/20"
